    ...
    <div class="items-center hidden gap-10 px-3 sm:flex ">
      <div class="flex justify-between gap-2 ">
        ...
        <div class="flex flex-col text-left">
          <div class="font-semibold">
            {{ sessionStore.getCurrentDate }}
          </div>
          <div class="text-sm">
            {{ sessionStore.getCurrentTime }}
          </div>
        </div>
      </div>
      <button
        ...
        @click="togglePopover"
      >
        <svg class="text-primary size-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div class="flex flex-col w-40 gap-2 p-2 overflow-hidden">
          <div class="flex flex-wrap justify-center gap-[10px]">
            <div
              ...
              :key="index"
              :class="[
                'w-5 h-5 rounded-full cursor-pointer outline outline-2 outline-offset-1 hover:scale-125 hover:outline-primary',
                `${themeStore.selectedColorScheme?.colorSchemeName === colorScheme.colorSchemeName ? 'outline-primary' : 'outline-transparent '}`
              ]"
              :style="{
          ...
          <button
            type="button"
            class="flex items-center gap-2 px-2 py-1 rounded bg-primary text-primary-contrast hover:bg-primary-emphasis"
            @click="loginStore.handleActionLogout"
          >
