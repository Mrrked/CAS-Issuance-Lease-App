...
export const useIssuanceStore = defineStore('issuance', () => {
  ...
  const convertInvoiceRecordsToInvoicePDFs = (selectedInvoiceRecord: InvoiceRecord, isDraft?: boolean): InvoicePDF => {
  ...
  const handlePreviewInvoicePDf = (invoicePDF: InvoicePDF) => {
    const PDF_BLOB = generateInvoicePDFBlob(invoicePDF)
