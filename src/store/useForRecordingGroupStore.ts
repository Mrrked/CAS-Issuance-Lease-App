import { FAILED_INVOICE_RECORDS, ForRecordingGroupForm, ForRecordingGroupOption, INVOICE_PER_COMPANY_AND_PROJECT, InvoiceDetails, InvoiceRecord, LeaseBill } from './types';
...
const SelectedBillsTableModal = defineAsyncComponent(() => import('../components/Dialog/ForRecordingGroup/SelectedBillsTableModal.vue'));

export const useForRecordingGroupStore = defineStore('ForRecordingGroup', () => {
  ...
  const BILL_TYPE_OPTIONS: ForRecordingGroupOption[] = [
    ...
    { value: 'C', name: '(C) Rental and CUSA',                billTypes: [1, 2, 4, 11, 21, 31, 41]},
    ...
    { value: 'H', name: '(H) Other Fees',                     billTypes: [10, 81, 83, 84]},
  ]
  ...
  const invoice_records_data = computed((): InvoiceRecord[] => {
    return issuanceStore.convertBillingsToInvoiceRecords(billings_data.value, forRecordingGroupForm.value.invoiceDate)
      .sort((a,b) => {
  ...
  const isShowPBLForm = computed((): boolean => {
    return (
      ['D','E','F','G'].includes(forRecordingGroupForm.value.billType.value)
    )
  })
  ...
  const canRunIssuance = computed(() => {
    const today = forRecordingGroupForm.value.invoiceDate.getDay()
    // Monday = 1, ..., Friday = 5
  ...
  const handleActionViewMainDialog = () => {
    ...
    const PerBatchRunDialog = dialog.open(SelectedBillsTableModal, {
      data: {
        invoice_date: forRecordingGroupForm.value.invoiceDate,
      ...
      props: {
        header: '(Batch) For Issuance of Invoice - ' + forRecordingGroupForm.value.invoiceDate.toLocaleString('en-US', { month: 'long' }),
        style: {
          width: '75vw'
        },
        showHeader: true,
  ...
  const handleActionIssueInvoices = async () => {
    ...
    const data = {
      year: forRecordingGroupForm.value.invoiceDate.getFullYear(),
      month: forRecordingGroupForm.value.invoiceDate.getMonth() + 1,
      day: forRecordingGroupForm.value.invoiceDate.getDate(),
      type: `From Recording Group (${forRecordingGroupForm.value.billType.value})`,
      ...
      const ShowResultFinalInvoiceDialog = dialog.open(ResultIssuedInvoicesModal, {
        data: {
          ...
          viewSummarySuccessInvoices: async () => {
            ...
            const header = '(Batch) Summary of Issued Invoices - ' + forRecordingGroupForm.value.invoiceDate.toLocaleString('en-US', { month: 'long', year: 'numeric' })
  ...
  // watch(
  //   () => forRecordingGroupForm.value.billType,
  //   () => {
  //     forRecordingGroupForm.value.PBL = {
  //       pcs_code: {
  //         1: '',
  //       },
  //       phase: {
