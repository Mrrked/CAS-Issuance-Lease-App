  ...
  const handleActionIssueInvoices = async () => {
    ...
    const SELECTED_INVOICES: InvoiceRecord[] = [
      ...invoice_records_data.value.map((INVOICE) => {
        ...
        const NO_OF_MONTHS = issuanceStore.getNOMOS(INVOICE, forBillingGroupForm.value.billType.billTypes)
          ...
          entry = {
            ...
            GFL1PF: {
              ...entry.GFL1PF,
                DATTRN: invoice_date,
                DATECR: invoice_date,
                PAYCOD: ['A','B'].includes(forBillingGroupForm.value.billType.value) ?
                  '8888' :
                  '9999',
                PAYEE: ['A','B'].includes(forBillingGroupForm.value.billType.value) ?
                  'N.A.' :
