<script lang="ts" setup>
  ...
  import { useBatchPDFSavingStore } from '../../store/useBatchPDFSavingStore';

  const mainStore = useMainStore()
  const utilStore = useUtilitiesStore()
  const batchSavingStore = useBatchPDFSavingStore()
  ...
  onMounted(() => {
    batchSavingStore.handleActionReset()
  })
</script>
...
<template>
  ...
  <div class="flex flex-col h-full">
    <DataTable
      :value="batchSavingStore.issued_documents_data"
      dataKey="INVOICE_KEY.INVOICE_NUMBER"
      v-model:selection="batchSavingStore.selectedIssuedDocuments"
      ...
      <template #header>
        <div class="flex items-center justify-between gap-4">
          <div class="flex gap-4 !text-xs items-center">
            <DatePicker
              v-model="batchSavingStore.batchPDFSavingForm.valueDate"
              view="month"
              dateFormat="MM yy"
              size="small"
            ...
            <Select
              ref="indexRef"
              v-model="batchSavingStore.batchPDFSavingForm.company"
            ...
            />
            <Button
              @click="batchSavingStore.handleActionSearch"
              icon="pi pi-search"
              type="button"
              severity="primary"
              label="SEARCH"
            ...
            />
            <Button
              @click="batchSavingStore.handleActionReset"
              icon="pi pi-times"
              type="button"
              severity="warn"
              label="RESET"
              class="!py-[7px]"
              v-if="batchSavingStore.issued_documents_data.length > 0"
            />
          </div>
          <Button
            @click="batchSavingStore.handleActionSaveSelectedInvoices"
            icon="pi pi-print"
            size="small"
            type="button"
            severity="primary"
            :label="'SAVE (' + batchSavingStore.selectedIssuedDocuments.length + ')'"
            :disabled="!batchSavingStore.selectedIssuedDocuments.length"
            class="h-8 w-44"
          />
        </div>
      </template>
